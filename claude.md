...
- `ContentLayout.astro` — Standard content page wrapper (takes optional `lastReviewed` Date prop → renders "Last reviewed" line + JSON-LD dateModified)
- `SectionIndexLayout.astro` — Section index pages

### Information architecture (July 2026 consolidation)
Two nav-level merges — **URLs never changed**, only navigation surfaces:
- **Reading LLM Writing** (`/reading/`) is the Verify hub: the skill, four named patterns, all antipatterns rendered as a gallery (from the `antipatterns` collection), and the specimen form. `/antipatterns/*` URLs stay live with a "part of Reading" banner; Antipatterns is not in nav.
- **Tasks** (`/tasks/`) is the Do hub: six task categories + the six workflows listed as a second tier. Each task page ends with a curated `## Related` section linking its prompts/workflow. `/workflows/*` URLs stay live with a banner; Workflows is not in nav.
- Desktop nav = 8 primary (Home, Foundations, Goals, Prompts, Tools, Tasks, Reading, Domains) + 8 in More. Adding a nav item means adding it to FOUR surfaces: desktop nav, mobile groups, footer, home explore grid.
- The foundations page at `/foundations/prompt-patterns/` is titled "Anatomy of a Clinical Prompt" (prompt mechanics); the `patterns/` collection is "Clinical Patterns" (clinical orientations). Don't reintroduce the name collision.
- `search-index.json.ts` and `llms-full.txt.ts` enumerate collections explicitly — **a new collection must be added to both** or it's invisible to site search and machine surfaces.
...
**Architecture.** All three share `public/tools/engine/engine.js` (analysis functions on `window.SLPIO.engine`), `public/tools/engine/data.js` (editorial tables: starter CMU subset, Crowe & McLeod 2020 acquisition ages, Beck tiers, articulation phoneme metadata, elicitation cues), and `public/tools/engine/cmu.js` (GENERATED — ~48k-entry pronunciation dictionary; see below). Script order in every tool page matters: `data.js` → `cmu.js` → `engine.js`, because cmu.js merges over data.js's starter dict and engine.js captures `SLPIO.CMU` by reference at load. All ship verbatim from `public/`, *not* through Astro's bundler. The inline IIFE in each page accesses the global and attaches DOM handlers. Shared styles live in `src/styles/tools.css`, imported per tool page.
...
- **Always show the math.** Counts, source attributions, and a coverage warning naming the words dictionary lookup missed (typically names, invented words, and clinical jargon — coverage on real-world text runs ~92–100%).
...
**The pronunciation dictionary.** `public/tools/engine/cmu.js` is generated by `node scripts/build-cmu-dict.mjs` (Node 22; sources auto-download into gitignored `scripts/.cache/`). It intersects cmudict-0.7b with Norvig's web-frequency list (top 40k), adds a contractions/possessives pass ("they're", "teacher's" — the frequency list strips apostrophes, so these need the base-word rule), and emits ~48k entries in frequency order (~1.35MB raw, ~430KB gzipped). Frequency order matters: `wordsWith()` iterates insertion order, so stimulability word lists come back most-common-first and the pages cap display at 48. The ~200 curated kid words in data.js stay first in insertion order deliberately. Don't edit cmu.js by hand — change the generator and rerun. Known bias: word lists skew web-English ("copyright" ranks high); a children's-corpus frequency source would be the next upgrade. `SLPIO.BECK_TIER` in data.js is still a small curated list — the check tool's copy says so; expanding it is editorial work, not a data swap.
...
- **Slow layer / fast layer.** Evergreen pages (foundations, glossary, domains) stay model-version-agnostic — "current major models," never "GPT-4 offers 128K tokens." Version-specific, volatile claims (BAA coverage, privacy defaults, new models) live in `ai-updates/` entries: dated, risk-labeled, source-linked. When an evergreen page must touch a volatile fact, it links the relevant AI Updates entry.
- **`lastReviewed` frontmatter** (optional, all collections): set it ONLY when a human or verified agent pass actually checked the entry against current reality that day. Never bulk-backfill. ContentLayout renders it when present.
- **`npm run freshness`** (scripts/freshness.mjs) is the monthly punch list: review ages + never-reviewed counts, then a live link check of every external citation (broken / bot-blocked / redirected). Node 22.
- **Citations must RESOLVE, not just look right.** A confabulated research digest (real author names, plausible venue, well-formed DOI that didn't exist) survived a plausibility review and was caught only by the link check in July 2026. When adding research entries: resolve the DOI, read the abstract, then write.
