 ...
 *
 * The full dictionary lives in cmu.js (generated by
 * scripts/build-cmu-dict.mjs, ~48k entries) and merges over the starter
 * CMU set below at load time. Keep the starter set: it seeds insertion
 * order with kid-friendly words (wordsWith returns them first) and keeps
 * the tools functional if cmu.js ever fails to load.
 *
 * TODO before v1:
 ...
 *   - Expand BECK_TIER from the current seed list using a curated
 *     corpus (e.g., Educator's Word Frequency Guide).
 */
...
/* Starter CMU-dict subset. Format:
 *   word: [ARPABET phoneme array]
 *
 * Stress markers stripped (CMU uses 0/1/2 on vowels — we don't need them).
 * cmu.js merges the full ~48k-entry dictionary over this at load time;
 * these ~200 curated kid words stay first in insertion order, which is why
 * stimulability word lists lead with storybook vocabulary.
 */
SLPIO.CMU = {
