...
slpDefinition: "Whether a goal can actually be tracked with data. Measurability is the make-or-break quality of any IEP or treatment goal. A measurable goal specifies what the client will do, under what conditions, and to what criterion — and a clinician can reliably collect data on it during a session. LLMs produce goals that sound measurable but often are not. The words are right, but the data collection is impossible or impractical."
...
1. **Apply the clipboard test** — before accepting any AI-generated goal, ask: "Can I take data on this with a pencil and a tally sheet during a real session?"
2. **Watch for vague conditions** — "in structured activities," "given moderate cues," "across contexts" are red flags for unmeasurable goals
3. **Define the denominator** — if the goal says "80% accuracy," you need to know 80% of what. If you can't define the total, the goal is not measurable
4. **Check the criterion against your schedule** — "across 3 consecutive sessions" means something different for a client you see weekly vs. monthly
